...
local function spell_castable(spell)
  ...
  if dark_addon.healthCooldown[calledUnit.unitID] ~= nil
  and dark_addon.healthCooldown[calledUnit.unitID] > GetTime() then
    onCooldown = true
  end
  dark_addon.console.debug(1, 'engine', 'engine', string.format('in unit:castable unit %s onCooldown %s', UnitName(calledUnit.unitID), tostring(onCooldown)))
